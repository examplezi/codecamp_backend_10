...
app.get("/boards", async (req, res) => {
  ...
  // ];
  const result2222 = await Board.find();
...
app.post("/boards", async (req, res) => {
  ...
  await board.res //3. DB에 저장된 결과를 브라우저에 응답(res) 주기 //저장
    .send("게시물 등록에 성공했습니다.");
});
...
// 몽고DB 접속, 네임 리졸루션
mongoose
  .connect("mongodb://my-database:27017/mydocker10") //도커이름이 적혀 있어야함 , name resolution
  .then(() => {
    console.log("접속완료");
  })
  .catch(() => {
