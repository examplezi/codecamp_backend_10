...
export class ProductsService {
  constructor(
    ...
    private readonly productsTagesRepository: Repository<ProductSaleslocation>
  ...
  async create({
    ...
    const { productSaleslocation, productTags, ...product } = createProductInput;
    // 2-1) 상품거래위치 등록
    ...
    // 2-2) 상품태그등록
    // => 상품태그예시? productTags = ["#전자제품", "#영등포", "#컴퓨터"]
    const temp = []
    for(let i =0; i < productTags.length; i++){
        const tagname =  productTags[i].replace("#", "") //전자제품
      
        const newTag = await this.productsTagesRepository.save({
          name: tagname,
        })
        temp.push(newTag)
    }
  ...
  async delete({ productId }: IProductsServiceDelete : void): Promise<boolean> {
    // 1. 실제 삭제
